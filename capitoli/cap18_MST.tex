...
10.         \textbf{if} \(e \text{is not None}\) \textbf{then}
11.             Add edge \(e\) to \(T\)
12.         \textbf{for each} edge \(e' = (u, v)\) such that \(v\) in \(Q\) \textbf{do}
13.             # check if edge \((u, v)\) better connects \(v\) to \(T\)
14.             \textbf{if} \(w(u, v) < D[v]\) \textbf{then}
15.                 \(D[v] = w(u, v)\)
16.                 Change the key of vertex \(v\) in \(Q\) to \(D[v]\).
17.                 Change the value of vertex \(v\) in \(Q\) to (\(v, e'\)).
