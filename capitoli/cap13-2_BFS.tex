...
\begin{enumerate}
    ...
    \item Se $(u,v)$ è un arco che non appartiene all'albero BFS, allora livello del vertice $v$ può essere al massimo $1$ maggiore del livello del vertice $u$.
\end{enumerate}
...
\begin{lstlisting}[
    ...
    def BFS_complete(g):
        """ Makes a BFS on the graph g and returns a dictionary where each vertex is mapped to the edge used to discover it (all the roots of the BFS trees are mapped to None)"""
        forest = {}
        for u in g.vertices():
            if u not in forest:
