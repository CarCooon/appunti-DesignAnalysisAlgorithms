...
L'inserimento eseguito in questo modo mantiene le Root, External, e Depth Properties:
\begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Si esegue una \textbf{ricolorazione} (recolouring) colorando $v$ e $w$ di nero, mentre $u$ viene colorato di rosso (se $u$ non è la radice).
...
\subsection{Delete di un nodo rosso}
Se il nodo eliminato è rosso, tutte le proprietà dei Red-Black Trees rimangono valide, poiché la rimozione di un nodo rosso non altera la black depth di alcun percorso dalla radice a una foglia, e poiché questa operazione non può introdurre un double red. Nel corrispondente (2, 4)-Tree, la rimozione di un nodo rosso equivale alla rimozione di una chiave da un 3-nodo o da un 4-nodo, il che è sempre consentito senza ulteriori modifiche.

\subsection{Delete di un nodo nero con un figlio rosso}
\uline{Ricordiamo che stiamo trattando la rimozione di nodi con al più un figlio (per via della delete in un BST).} Se il nodo da eliminare è nero e ha un figlio, questo sarà sicuramente rosso (altrimenti la black depth property non sarebbe soddisfatta e non si avrebbe un Red-Black Tree valido).

In questo caso, possiamo semplicemente rimuovere il nodo nero e promuovere il figlio rosso al suo posto, colorandolo di nero, ristabilendo tutte le proprietà dei Red-Black Trees. Nel corrispondente (2, 4)-Tree, questa operazione equivale alla rimozione del nodo nero da un 3-nodo.

\vspace{8\baselineskip}
\noindent
Infine, consideriamo il caso più complesso, in cui il nodo da eliminare è un nodo nero foglia(None).

\clearpage
\subsection{Delete di un nodo nero foglia (None)}
Il caso più complesso si verifica quando il nodo da eliminare è un nodo nero foglia (None). Nel corrispondente (2, 4)-Tree, questa situazione equivale alla rimozione di una chiave da un 2-nodo. Senza un ribilanciamento, una modifica del genere comporta un deficit di uno per la black depth lungo il percorso che porta al nodo eliminato, violando così la Depth Property dei Red-Black Trees. Se prima della cancellazione l'albero era un Red-Black Tree valido, il nodo rimosso avrà sicuramente un fratello (sibling) il cui sottoalbero (radicato nel fratello) ha black height uguale a 1.
...
\begin{itemize}
    \item Nodo $y$ nero con un figlio rosso $x$.
    \item Nodo $y$ nero e entrambi i figli di $y$ sono neri (o None).
    \item Nodo $y$ rosso.
\end{itemize}

\clearpage
\subsubsection{Caso 1: Nodo $y$ nero con un figlio rosso $x$}
Si esegue una \emph{ristrutturazione} \texttt{restructure(x)} sui tre nodi $x$, il suo genitore $y$, e il nonno $z$, rinominati temporaneamente come $a$, $b$, e $c$ in ordine di chiave. 
Sostituiamo $z$ con il nodo etichettato $b$, rendendolo il genitore degli altri due. Coloriamo $a$ e $c$ di nero, e diamo a $b$ il colore precedente di $z$.

\vspace{1\baselineskip}
Da notare che il percorso verso $T_{\text{light}}$ include un nodo nero aggiuntivo dopo la ristrutturazione, risolvendo così il suo deficit. Al contrario, il numero di nodi neri sui percorsi verso ciascuno degli altri tre sottoalberi illustrati in Figura \ref{fig:RBT_delete_case1} rimane invariato.
...
\clearpage
\subsubsection{Caso 2: Nodo $y$ nero e entrambi i figli di $y$ sono neri (o None)}
La soluzione in questo caso corrisponde all'operazione di \emph{fusion} nell'albero (2, 4) $T'$, poiché $y$ deve rappresentare un 2-nodo. Eseguiamo una \emph{ricolorazione}; coloriamo $y$ di rosso e, se $z$ è rosso, lo coloriamo di nero. (Vedi Figura \ref{fig:RBT_delete_case2}). Questo non introduce alcuna violazione rossa, poiché $y$ non ha figli rossi.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{immagini/RBT/RBT_delete_case2.png}
...
\clearpage
\subsubsection{Caso 3: Nodo $y$ rosso}
Dato che $y$ è rosso e $T_{\text{heavy}}$ ha altezza nera almeno 1, $z$ deve essere nero e i due sottoalberi di $y$ devono avere ciascuno una radice nera e un'altezza nera uguale a quella di $T_{\text{heavy}}$. In questo caso, eseguiamo una rotazione su $y$ e $z$, e poi ricoloriamo $y$ di nero e $z$ di rosso. Questo denota una riorientazione di un 3-nodo nel corrispondente albero (2,4) $T'$.

Questo non risolve immediatamente il deficit, poiché il nuovo sottoalbero di $z$ è un vecchio sottoalbero di $y$ con radice nera $y'$ e altezza nera uguale a quella dell'originale $T_{\text{heavy}}$. Riapplichiamo l'algoritmo per risolvere il deficit in $z$, sapendo che il nuovo figlio $y'$, che è la radice di $T_{\text{heavy}}$ è ora nero, e quindi che si applica o il Caso 1 o il Caso 2. Inoltre, la prossima applicazione sarà l'ultima, perché il Caso 1 è sempre terminale e il Caso 2 sarà terminale dato che $z$ è rosso.
