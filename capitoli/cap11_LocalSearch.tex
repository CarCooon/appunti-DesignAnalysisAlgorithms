...
Consideriamo un nodo con stato $-1$ connesso ad altri nodi. Se il "peso" dei conflitti (archi cattivi) supera quello degli accordi, il nodo inverte il suo stato a $+1$. Questo cambiamento rende "buoni" gli archi incidenti che prima erano cattivi, e viceversa.

\subsection{Convergenza e Complessità}
Una domanda fondamentale è: l'algoritmo termina sempre o può entrare in un ciclo infinito di cambi di stato?
È possibile dimostrare che l'algoritmo termina sempre.

\paragraph{Teorema di Convergenza.}
L'algoritmo State-flipping termina con una configurazione stabile dopo al più $W = \sum_{e} |w_e|$ iterazioni.

\paragraph{Dimostrazione.}
Consideriamo una funzione potenziale $\Phi(S)$ definita come la somma dei pesi assoluti di tutti gli archi "buoni" nella configurazione corrente:
...
\begin{itemize}
    ...
    $$ \Phi(S') \ge \Phi(S) + 1 $$
\end{itemize}
Poiché la funzione potenziale è limitata superiormente e cresce strettamente ad ogni passo, l'algoritmo deve terminare.

\paragraph{Note sulla complessità.}
Sebbene la convergenza sia garantita, il numero di passi può essere molto grande.
\begin{itemize}
    \item Il \textbf{problema decisionale} ("Esiste una configurazione stabile?") è banale: la risposta è sempre sì, poiché l'algoritmo ne trova sempre una.
    \item Il \textbf{problema di ricerca} ("Trova una configurazione stabile") appartiene alla classe \textbf{PLS} (Polynomial Local Search). Non è noto alcun algoritmo in tempo polinomiale (nel caso peggiore) per risolvere questo problema.
\end{itemize}
